...
public class DataBase {
    ...
    public DataBase() throws IOException {
        ...
        Patient patient1 = new Patient(301, "Anjali Gupta", "1234567890", "123, Main Street, City", "O+", "Female", 35,
                "Cardiology", 101, 201);
        patData.add(patient1);

        Patient patient2 = new Patient(302, "Amit Singh", "9876543210", "456, Second Avenue, Town", "A-", "Male", 45,
                "Rheumatology", 102, 202);
    ...
    public void removePatient(int pat_ID) {
        ...
        while (iterator.hasNext()) {
            ...
            if (pat.get_Id() == pat_ID) {
                ...
                /*
                 * After the removal of the patients Id from the PatientsAssigned ArrayList of
                 * the doctors
    ...
    public void removeDoctor(int doc_ID) {
        ...
        while (iterator.hasNext()) {
            ...
            if (doc.get_Id() == doc_ID) {
                ...
                /*
                 * When a doctor is removed, we change the doctor_alloted field in respective
                 * Patient to -1 indicating no doctor is alloted.
                 */

                for (int patToRemove_Id : doc.PatientsAssignedToDoc) {
                    Iterator<Patient> pat_Iter = patData.iterator();
                    while (pat_Iter.hasNext()) {
                        Patient pat = pat_Iter.next();
                        if (pat.get_Id() == patToRemove_Id) {
                            pat.setAllotedDoctor(-1);
                            break;
                        }
                    }
                }
    ...
    public void removeNurse(int nur_ID) {
        ...
        while (iterator.hasNext()) {
            ...
            if (nur.get_Id() == nur_ID) {
                ...
                for (int patToRemove_Id : nur.PatientsAssignedToNur) {
                    Iterator<Patient> pat_Iter = patData.iterator();
                    while (pat_Iter.hasNext()) {
                        Patient pat = pat_Iter.next();
                        if (pat.get_Id() == patToRemove_Id) {
                            pat.setAllotedNurse(-1);
                            break;
                        }
                    }
                }
    ...
    public int assign_Doctor(String dept, int patId) {
         ...
         * assign_Doctor() assigns the ID of a Doctor whose department matches with
         * the
         * Patient's health issues on the basis of the String dept.
         */
